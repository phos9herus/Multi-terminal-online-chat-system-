...
def append_reaction_to_file(folder, msg_id, reaction_type):
    os.makedirs(folder, exist_ok=True)
...
def load_all_reactions():
    ...
    message_reactions = {}  # 重置

    # 遍历 chat_logs 下的所有子文件夹
        ...
        for root, dirs, files in os.walk(LOGS_DIR):
            if 'reactions.csv' in files:
                ...
                try:
                    ...
                    print(f"[REACTION LOAD ERROR] {path}: {e}")
...
def handle_reaction(data):
    ...
    if not target_uid or target_uid == 'global':
        ...
        # 私聊：根据 sender 和 target 确定文件夹 (u1_u2)
        u1, u2 = sorted([str(sender_uid), str(target_uid)])
        folder = os.path.join(LOGS_DIR, f"{u1}_{u2}")

    # 保存到对应的 CSV
    append_reaction_to_file(folder, msg_id, reaction_type)
    ...
    if not target_uid or target_uid == 'global':
        emit('reaction_update', payload, to='global_chat')
    else:
        # 私聊：发给双方
        emit('reaction_update', payload, room=sid)  # 自己
        if target_uid in uid_to_sid:
            emit('reaction_update', payload, room=uid_to_sid[target_uid])  # 对方
        # 也发给 Admin 监控
        emit('reaction_update', payload, to='admin_room')
